        ...
        def do_GET(self):
            ...
            if os.access("{}/{}".format(folder, sciezka), os.R_OK):
                ...
                if czyJestPlikiem:
                    ...
                    linie = subprocess.run(["ls", "-la", "."], capture_output=True, encoding="utf8").stdout.rstrip().split("\n")
                    self.wfile.write(bytes("<html><body>", "utf-8"))
                    for linia in linie[1:]:
                        linia = linia.split(" ")
                        #print(linia)
                        self.wfile.write(bytes("<p>{}   {}  {} {}  <a href=\"{}\">{}</a></p>".format(linia[0][0], linia[-4], linia[-3], linia[-2], linia[-1], linia[-1]), "utf-8"))
